...
from prob_utils import ProbabilisticHead, free_energy_distance, load_prob_head, cope_distance
...
def compute_fiq_val_metrics(relative_val_dataset: FashionIQDataset, clip_model: CLIP, index_features: torch.tensor,
                            ...
                            prob_head: ProbabilisticHead = None,
                            distance_fn: callable = free_energy_distance) -> Tuple[float, float]:
    """
    ...
    :param combining_function: function which takes as input (image_features, text_features) and outputs the combined
                            features
    :param prob_head: when provided, rank by free energy instead of the dot product
    ...
    if prob_head is not None:
        with torch.no_grad():
            ...
            index_log_var = prob_head.target_log_var(index_features)
            distances = distance_fn(predicted_features, query_log_var, index_features, index_log_var)
    else:
...
def fashioniq_val_retrieval(dress_type: str, combining_function: callable, clip_model: CLIP, preprocess: callable,
                            prob_head: ProbabilisticHead = None,
                            distance_fn: callable = free_energy_distance):
    """
    ...
    :param prob_head: when provided, rank by a probabilistic distance instead of the dot product
    :param distance_fn: distance used for ranking when prob_head is provided
    """
...
def compute_cirr_val_metrics(relative_val_dataset: CIRRDataset, clip_model: CLIP, index_features: torch.tensor,
                             index_names: List[str], combining_function: callable,
                             prob_head: ProbabilisticHead = None,
                             distance_fn: callable = free_energy_distance) -> Tuple[
...
def main():
    ...
    parser.add_argument("--prob-head-path", type=Path,
                        help="Path to a trained ProbabilisticHead; when given, ranking uses a probabilistic distance")
    parser.add_argument("--relevance", default="free_energy", type=str, choices=['free_energy', 'cope'],
                        help="Probabilistic distance for ranking: 'free_energy' (KL) or 'cope'")
    ...
    if args.prob_head_path:
        from prob_utils import get_prob_distances
        _, distance_fn = get_prob_distances(args.relevance)
        print(f'Loading the ProbabilisticHead, ranking will use {args.relevance} distance')
        prob_head = load_prob_head(args.prob_head_path, feature_dim, device)

    if args.dataset.lower() == 'cirr':
        group_recall_at1, group_recall_at2, group_recall_at3, recall_at1, recall_at5, recall_at10, recall_at50 = \
            cirr_val_retrieval(combining_function, clip_model, preprocess, prob_head, distance_fn)
        ...
        shirt_recallat10, shirt_recallat50 = fashioniq_val_retrieval('shirt', combining_function, clip_model,
                                                                     preprocess, prob_head, distance_fn)
