...
def clip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
                      ...
                      relevance: str = 'dot', prob_lr: float = 1e-4, free_energy_scale: float = 10.0,
                      **kwargs):
    """
    ...
    :param relevance: 'dot' for the original dot-product logits, 'free_energy' for the probabilistic variant
    :param prob_lr: learning rate for the variance heads when relevance == 'free_energy'
    :param free_energy_scale: scale applied to the negative free energy before the cross-entropy loss
    :param kwargs: if you use the `targetpad` transform you should prove `target_ratio` as kwarg
    """

    training_start = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    relevance_tag = relevance if relevance in ('dot', 'free_energy', 'cope') else 'dot'
    training_path: Path = Path(
        base_path / f"models/clip_finetuned_on_fiq_{clip_model_name}_{relevance_tag}_{training_start}")
    ...
    if use_prob:
        ...
        prob_head = ProbabilisticHead(clip_model.visual.output_dim).to(device, non_blocking=True)
    else:
        print('Relevance: dot product (baseline)')

    # Define the optimizer, the loss and the grad scaler
    param_groups = [{'params': filter(lambda p: p.requires_grad, clip_model.parameters()), 'lr': learning_rate,
        ...
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions) in enumerate(train_bar):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if use_prob:
                        query_log_var = prob_head.query_log_var(predicted_features)
                        target_log_var = prob_head.target_log_var(target_features)
                        logits = logits_fn(predicted_features, query_log_var,
                                           target_features, target_log_var,
            ...
            with experiment.validate():
                ...
                for relative_val_dataset, classic_val_dataset, idx in zip(relative_val_datasets, classic_val_datasets,
                    ...
                    recall_at10, recall_at50 = compute_fiq_val_metrics(relative_val_dataset, clip_model,
                                                                       ...
                                                                       combining_function, prob_head, distance_fn)
...
def clip_finetune_cirr(num_epochs: int, clip_model_name: str, learning_rate: float, batch_size: int,
    ...
    relevance_tag = relevance if relevance in ('dot', 'free_energy', 'cope') else 'dot'
    training_path: Path = Path(
        base_path / f"models/clip_finetuned_on_cirr_{clip_model_name}_{relevance_tag}_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
